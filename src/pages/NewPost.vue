<template>
  <div class="flex flex-col md:flex-row">
    ...
    >
      ...
      <form
        class="bg-white rounded shadow p-10 justify-center xl:mx-1 2xl:mx-10 lg:my-5"
      >
    ...
    <div class="flex-1 h-screen scrollbar-hide 2xl:h-screen overflow-scroll">
      <div
        class="flex flex-1 flex-col justify-center items-center m-10"
        v-if="!post.title"
      >
        <vue-feather
          type="alert-triangle"
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      this.post.authorEmail = this.authUser.email;
      let flag = postsList.filter((post, index) => {
        if (post.slug === this.post.slug) {
          // the post already exists
          return (postsList[index] = this.post);
        }
      });

      if (flag.length == 0) {
        postsList.push(this.post);
      }
